...
main() {
    ...
    if [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "win32" ]] || [[ "$OS" == "Windows_NT" ]]; then
        ...
        # Check for Windows scripts first (for cross-platform testing)
        if check_script_exists "sync-vault.ps1"; then
            print_info "Found PowerShell script - attempting to run via pwsh"
            if command -v pwsh &> /dev/null; then
                pwsh -ExecutionPolicy Bypass -File "sync-vault.ps1"
                SYNC_EXIT_CODE=$?
            else
                print_warning "PowerShell Core not installed, trying bash script"
                if check_script_exists "sync-vault.sh"; then
                    bash "sync-vault.sh"
                    SYNC_EXIT_CODE=$?
                else
                    print_error "Neither PowerShell nor bash script found"
                    exit 1
                fi
            fi
        elif check_script_exists "sync-vault.sh"; then
